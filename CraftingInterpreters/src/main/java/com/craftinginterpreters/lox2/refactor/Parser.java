...
import com.craftinginterpreters.lox2.refactor.ObjFunction.Type;
...
public class Parser {
    ...
    private void classDeclaration() {
        ...
        byte nameConstant = compiler.identifierConstant(previous.lexeme);
        compiler.declareVariable();

        compiler.emitBytes(OpCode.OP_CLASS, nameConstant);
        compiler.defineVariable(nameConstant);

        compiler.currentClass = new ClassCompiler(compiler.currentClass);

        if (match(TokenType.LESS)) {
            consume(TokenType.IDENTIFIER, "Expect superclass name.");
            variable(false);

            if (className.lexeme.equals(previous.lexeme)) {
    ...
    private void method() {
        ...
        byte constant = compiler.identifierConstant(previous.lexeme);

        Type functionType = ObjFunction.Type.METHOD;
        if (previous.lexeme.equals("init")) {
            functionType = ObjFunction.Type.INITIALIZER;
        }

        function(functionType);
        compiler.emitBytes(OpCode.OP_METHOD, constant);
    }

    private void funDeclaration() {
        byte global = compiler.parseVariable("Expect function name.");
        compiler.markInitialized();
        function(ObjFunction.Type.FUNCTION);
        compiler.defineVariable(global);
    }

    private void function(ObjFunction.Type functionType) {
        compiler = compiler.createNested(functionType);
    ...
    private void returnStatement() {
        ...
        if (match(TokenType.SEMICOLON)) {
            ...
            if (compiler.function.type == ObjFunction.Type.INITIALIZER) {
    ...
    private void namedVariable(String name, boolean canAssign) {
        OpCode getOp = OpCode.OP_GET_LOCAL;
    ...
    private void dot(boolean canAssign) {
        ...
        byte name = compiler.identifierConstant(previous.lexeme);

        if (canAssign && match(TokenType.EQUAL)) {
            expression();
            compiler.emitBytes(OpCode.OP_SET_PROPERTY, name);
        } else if (match(TokenType.LEFT_PAREN)) {
            byte argCount = argumentList();
            compiler.emitBytes(OpCode.OP_INVOKE, name);
            compiler.emitByte(argCount);
        } else {
            compiler.emitBytes(OpCode.OP_GET_PROPERTY, name);
        }
    }
    ...
    private void super_(boolean canAssign) {
        ...
        if (match(TokenType.LEFT_PAREN)) {
            ...
            compiler.emitBytes(OpCode.OP_SUPER_INVOKE, name);
            compiler.emitByte(argCount);
        } else {
            namedVariable("super", false);
            compiler.emitBytes(OpCode.OP_GET_SUPER, name);
        }
    }
    ...
    private final Consumer<Boolean> DOT = (b) -> dot(b);
    private final Consumer<Boolean> THIS_ = (b) -> this_(b);
    private final Consumer<Boolean> SUPER_ = (b) -> super_(b);
    ...
    private ParseRule[] rules = { // @formatter:off
        ...
        /* DOT.......... */ new ParseRule(null,     DOT,    Precedence.CALL),
        ...
        /* SUPER........ */ new ParseRule(SUPER_,   null,   Precedence.NONE),
        /* THIS......... */ new ParseRule(THIS_,    null,   Precedence.NONE),
